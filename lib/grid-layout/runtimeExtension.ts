...
export type GridLayoutRuntimeExtension = {
  ...
  getItemRenderState: (
    item: LayoutItem,
    defaults: GridItemRenderDefaults,
...
export const getDefaultGridItemRenderState = (
  item: LayoutItem,
  defaults: GridItemRenderDefaults
): GridItemRenderState => {
  const draggable = typeof item.isDraggable === "boolean"
    ? item.isDraggable
    : !item.static && defaults.isDraggable;
  const resizable = typeof item.isResizable === "boolean"
    ? item.isResizable
    : !item.static && defaults.isResizable;

  return {
    visible: true,
    draggable,
    resizable,
    bounded: draggable && defaults.isBounded && item.isBounded !== false,
    className: undefined,
    previewItem: null,
    onClick: undefined
  };
...
export const createNoopGridInteractionsEditor = (): GridInteractionsEditor => ({
  clearGuides: () => undefined,
  resetSnap: () => undefined,
  snapCandidate: (_activeId: string, _activeItem: LayoutItem, candidateItem: LayoutItem) => candidateItem,
