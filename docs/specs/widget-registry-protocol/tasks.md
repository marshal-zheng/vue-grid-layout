...
- [x] 1. 搭建 `lib/widget-registry/` 基础结构与公共类型
...
- [x] 2. 实现 registry core 的注册、查询、过滤与 lifecycle diagnostics
...
- [x] 3. 实现 layout defaults 与 item capability bridge
...
- [x] 4. 实现 settings descriptor 默认值、校验与 declarative predicate
...
- [x] 5. 实现 widget template materialization
...
- [x] 6. 实现 dashboard document `extensions.widget` sidecar helpers
...
- [x] 7. 实现 dashboard editor shell adapter 桥接
...
- [x] 8. 同步 public entry、package exports、类型与 bundle boundary
...
- [x] 11. 建立 headless dogfood fixture/workbench
...
- [x] 14. 修复 Round 1 independent review 发现的 registry/shell alignment 问题
  - [x] duplicate registration 在 tolerant policy 下不得覆盖既有 definition，除非显式 `allowOverride`。
  - [x] paste widget prepare 必须能从 copy payload 克隆 registry metadata，而不是依赖 paste command 的空 itemIds。
  - [x] interactive `addWidgetFromTemplate()` 必须走 widget adapter prepare/commit/rollback，确保 placement commit 后写入 `extensions.widget`。
  - [x] widget adapter commit 不得把完整 widget sidecar 自动写入 profile override。
  - [x] descriptor-less settings validation 不得把已清理的 unsafe keys 从原始输入重新写回。
  - [x] hidden lifecycle status 必须通过 diagnostics 暴露，供 palette/resolve/materialize 可观察。
  _需求追溯: R1.AC2, R5.AC4, R6.AC2, R6.AC3, R6.AC4, R7.AC2, R7.AC3_
