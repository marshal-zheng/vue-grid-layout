# CORE-foundation-modernization Legacy Spec Brief
...
TARGET_SPEC: docs/specs/foundation-modernization
...
- `foundation-modernization`: Legacy migration coverage for `foundation-modernization` (Foundation Modernization 需求规格). Current initiative status: `done`. Existing requirements map to R1-R14.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/foundation-modernization/requirements.md`。
