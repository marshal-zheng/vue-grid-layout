# CORE-vue-grid-layout-behavior-preserving-refactor Legacy Spec Brief
...
TARGET_SPEC: docs/specs/vue-grid-layout-behavior-preserving-refactor
...
- `behavior-refactor`: Legacy migration coverage for `vue-grid-layout-behavior-preserving-refactor` (Vue Grid Layout 行为保持型组件边界重构 需求规格). Current initiative status: `done`. Existing requirements map to R1-R10.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/vue-grid-layout-behavior-preserving-refactor/requirements.md`。
