# CORE-height-modes-render-precision Legacy Spec Brief
...
TARGET_SPEC: docs/specs/height-modes-render-precision
...
- `height-render-runtime`: Legacy migration coverage for `height-modes-render-precision` (Height Modes & Render Precision 需求规格). Current initiative status: `done`. Existing requirements map to R1-R12.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/height-modes-render-precision/requirements.md`。
