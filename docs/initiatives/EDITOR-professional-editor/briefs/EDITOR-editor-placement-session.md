# EDITOR-editor-placement-session Legacy Spec Brief
...
TARGET_SPEC: docs/specs/editor-placement-session
...
- `editor-placement-session`: Legacy migration coverage for `editor-placement-session` (Editor Placement Session 需求规格). Current initiative status: `in-spec`. Existing requirements map to R1-R10.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/editor-placement-session/requirements.md`。
