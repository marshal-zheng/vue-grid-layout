# EDITOR-editor-command-kernel Legacy Spec Brief
...
TARGET_SPEC: docs/specs/editor-command-kernel
...
- `editor-command-kernel`: Legacy migration coverage for `editor-command-kernel` (Editor Command Kernel 需求规格). Current initiative status: `done`. Existing requirements map to R1-R12.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/editor-command-kernel/requirements.md`。
