# LAYOUT-external-drop-session-refactor Legacy Spec Brief
...
TARGET_SPEC: docs/specs/external-drop-session-refactor
...
- `external-drop-session`: Legacy migration coverage for `external-drop-session-refactor` (External Drop Session Refactor 需求规格). Current initiative status: `in-spec`. Existing requirements map to R1-R8.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/external-drop-session-refactor/requirements.md`。
