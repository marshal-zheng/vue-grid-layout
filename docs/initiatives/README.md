...
当前下一步：继续推进 DASH 的 `command-history-first-dashboard-editing`，从 `docs/specs/command-history-first-dashboard-editing/tasks.md` 的 T8 public exports、README 与迁移说明开始。理由是 T1 editor event/checkpoint/rollback 基础、T2 durable sidecar 写回、T3 shell-managed write-back opt-in API、T4 shell command commit coordinator、T5 pointer/resize/drop synthetic result、T6 undo/redo shell write-back 路径，以及 T7 dogfood workbench/result 面板迁移已完成；剩余 LAYOUT `external-drop-session` 与 `layout-engine-pro` 仍分别有 ownership / first-slice 开放问题，而 DASH command-history-first 已有 formal spec、依赖齐备、能稳定 dashboard shell 的 history/write-back 主线。
...
| DASH | Dashboard 平台与文档生命周期 | active | docs/initiatives/DASH-dashboard-platform | document/profile/shell 主线已完成；command-history-first 已完成 T1-T7，pure runtime split 仍未完成 | 继续 `command-history-first-dashboard-editing` T8 |
