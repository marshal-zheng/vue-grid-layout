...
当前下一步：推进 DASH 的 `command-history-first-dashboard-editing`，从 `docs/specs/command-history-first-dashboard-editing/tasks.md` 的 T1 editor event/checkpoint/rollback 基础开始。理由是 LAYOUT 的 `item-capabilities-aspect-ratio` 已完成，关键 capability / aspect ratio 前置依赖已解除；剩余 LAYOUT `external-drop-session` 与 `layout-engine-pro` 仍分别有 ownership / first-slice 开放问题，而 DASH command-history-first 已有 formal spec、依赖齐备、能稳定 dashboard shell 的 history/write-back 主线。
...
| DASH | Dashboard 平台与文档生命周期 | active | docs/initiatives/DASH-dashboard-platform | document/profile/shell 主线已完成；command-history-first 和 pure runtime split 仍未完成 | 推进 `command-history-first-dashboard-editing` |
