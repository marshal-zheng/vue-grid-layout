# DASH-responsive-dashboard-profiles Legacy Spec Brief
...
TARGET_SPEC: docs/specs/responsive-dashboard-profiles
...
- `responsive-profiles`: Legacy migration coverage for `responsive-dashboard-profiles` (Responsive Dashboard Profiles 需求规格). Current initiative status: `done`. Existing requirements map to R1-R9.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/responsive-dashboard-profiles/requirements.md`。
