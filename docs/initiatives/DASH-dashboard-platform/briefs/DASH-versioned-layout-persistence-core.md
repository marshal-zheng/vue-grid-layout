# DASH-versioned-layout-persistence-core Legacy Spec Brief
...
TARGET_SPEC: docs/specs/versioned-layout-persistence-core
...
- `versioned-persistence`: Legacy migration coverage for `versioned-layout-persistence-core` (版本化布局持久化核心 需求规格). Current initiative status: `done`. Existing requirements map to R1-R8.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/versioned-layout-persistence-core/requirements.md`。
