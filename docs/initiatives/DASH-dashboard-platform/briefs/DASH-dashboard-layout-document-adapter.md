# DASH-dashboard-layout-document-adapter Legacy Spec Brief
...
TARGET_SPEC: docs/specs/dashboard-layout-document-adapter
...
- `dashboard-document-adapter`: Legacy migration coverage for `dashboard-layout-document-adapter` (Dashboard Layout Document & Adapter 需求规格). Current initiative status: `done`. Existing requirements map to R1-R8.
...
无。Legacy migration 没有引入新的 blocking question；历史 clarification 仍保留在 `docs/specs/dashboard-layout-document-adapter/requirements.md`。
