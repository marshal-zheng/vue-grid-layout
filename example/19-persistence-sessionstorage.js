...
import { PersistentGridLayout, sessionStorageAdapter } from "@marsio/vue-grid-layout/persistence";

const VueGridLayout = WidthProvider(PersistentGridLayout);

const storagePrefix = "vgl:";
const storageKey = "persistence-sessionstorage-demo";
...
const App = {
  setup() {
    ...
    const adapter = sessionStorageAdapter({ prefix: storagePrefix });
    ...
    const readStorage = () => {
      const raw = window.sessionStorage.getItem(`${storagePrefix}${storageKey}`);
      if (!raw) {
    ...
    const clearSavedLayout = () => {
      window.sessionStorage.removeItem(`${storagePrefix}${storageKey}`);
    ...
    const corruptSavedLayout = () => {
      window.sessionStorage.setItem(`${storagePrefix}${storageKey}`, "{broken-json");
