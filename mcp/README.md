...
`@marsio/vue-grid-layout` 的 MCP 服务器，给 AI IDE 提供 lean root/core、responsive subpath、advanced wrapper、Props、类型定义和场景示例。

> 数据来源：直接基于仓库 `lib/VueGridLayout*.tsx` 的 props 定义和生成后的 `dist/types/*.d.ts` 公共类型声明；未构建时回退到源码类型。
...
| `list_vue_grid_layout_props` | 列出 lean 组件 props 的结构化信息（JSON，含描述与 kebab-case 名；ResponsiveVueGridLayout 会标记 `forwardedViaAttrs`） | `component?`: VueGridLayout / ResponsiveVueGridLayout / WidthProvider |
