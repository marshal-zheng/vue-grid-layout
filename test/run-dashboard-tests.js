...
const tmp = path.join(root, '.tmp', 'dashboard-tests');
...
run(process.execPath, [path.join(tmp, 'test', 'dashboard-core.test.js')]);
run(process.execPath, ['script.js', '--action=build']);
run(process.execPath, [path.join(root, 'test', 'dashboard-responsive-component-browser.test.js')]);
